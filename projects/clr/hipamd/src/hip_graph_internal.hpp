 ...
 public:
  ...
  hipError_t CreateCommand(hip::Stream* stream) override {
    ...
    amd::NDRangeContainer sizes = amd::MakeLaunchFromGrid(
        kernelParams_.gridDim.x, kernelParams_.gridDim.y, kernelParams_.gridDim.z,
        kernelParams_.blockDim.x, kernelParams_.blockDim.y, kernelParams_.blockDim.z,
        globalWorkSizeX_remainder_, globalWorkSizeY_remainder_, globalWorkSizeZ_remainder_,
        clusterDim_.x, clusterDim_.y, clusterDim_.z, valid);

    if (!valid) {
      return hipErrorInvalidConfiguration;
    }

    status = ihipLaunchKernelCommand(command, func, sizes, kernelParams_.sharedMemBytes, stream,
                                     kernelParams_.kernelParams, kernelParams_.extra,
                                     kernelEvents_.startEvent_, kernelEvents_.stopEvent_, flags,
                                     coopKernel_, 0, 0, 0, 0, 0);
  ...
  hipError_t SetAttrParams(hipKernelNodeAttrID attr, const hipKernelNodeAttrValue* params) {
    ...
    if (attr == hipKernelNodeAttributeAccessPolicyWindow) {
      ...
      amd::NDRangeContainer sizes = amd::MakeLaunchFromGrid(
          ...
          kernelParams_.blockDim.x, kernelParams_.blockDim.y, kernelParams_.blockDim.z,
          globalWorkSizeX_remainder_, globalWorkSizeY_remainder_, globalWorkSizeZ_remainder_,
          clusterDim.x, clusterDim.y, clusterDim.z, valid);
      if (!valid) {
        return hipErrorInvalidConfiguration;
      }
      clusterDim_ = clusterDim;
  ...
  hipError_t validateKernelParams(const hipKernelNodeParams* pNodeParams,
    ...
    bool valid;
    amd::NDRangeContainer sizes = amd::MakeLaunchFromGrid(
        pNodeParams->gridDim.x, pNodeParams->gridDim.y, pNodeParams->gridDim.z,
    ...
    hipError_t status = ihipLaunchKernel_validate(func, sizes, pNodeParams->sharedMemBytes,

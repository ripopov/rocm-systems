  ...
  if constexpr (std::is_same_v<AqlPacket, hsa_amd_ext_kernel_dispatch_packet_t>) {
    if (kAmdFormat4Supported) {
      ...
      if (amd_format == HSA_AMD_PACKET_TYPE_EXT_KERNEL_DISPATCH_LD) {
        auto* metadata = metadata_preloader_.GetMetadataPacket(index & queueMask);
        if (metadata != nullptr) {
...
bool VirtualGPU::submitKernelInternal(const amd::NDRangeContainer& sizes, const amd::Kernel& kernel,
  ...
  // Copy scheduler's AQL packet for possible relaunch from the scheduler itself
    ...
    if (extDispatchPacket) {
      ...
      ClPrint(amd::LOG_DEBUG, amd::LOG_AQL, "LD_DEBUG: amd_format=%d", static_cast<int>(amd_format));

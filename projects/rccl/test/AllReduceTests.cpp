...
namespace RcclUnitTesting
{
  ...
  // Exercises the packed 2-wide fp8 reduction helpers added for gfx942/gfx950
  // (hminmax2/hminmax2_b in rccl_float8.h). MinMax keeps the fp8 values in
  // range, so it validates the packed convert -> v_pk_{min,max}_f16 -> convert
  // path without the overflow concerns of fp8 Sum/Prod. Runs out-of-place,
  // non-graph, at both a large (packed hot loop) and small element count.
  TEST(AllReduce, Fp8PackedMinMax)
  {
    TestBed testBed;

...
int SimulatedKfd::debug_trap_ioctl(KfdProcess &caller, void *arg) {
  ...
  case KFD_IOC_DBG_TRAP_DISABLE:
    ...
    debug_sessions_.erase(target_pid);
    return 0;
  // Recognized ops whose handlers are not wired up yet.
...
int SimulatedKfd::debug_device_snapshot(kfd_ioctl_dbg_trap_device_snapshot_args &args) {
  ...
  for (uint32_t i = 0; i < fill; ++i) {
    ...
    const kmd::TopologyProperties topology =
        kmd::topology_properties_for(info.gfx_target_version, info.capability, info.capability2,
